...
try:
	files_in_convert = bool(os.listdir("convert"))
except FileNotFoundError:
	files_in_convert = False

if files_in_convert:
	print("Files found in \"convert\" folder. Converting files there.")

	file = []
	meta = []

	for i in os.listdir("convert"):
		if i[-4:] == ".tmx":
			# Copy file level
			with open("convert/" + i) as f:
				file.append(f.read())
		else:
			file.append("")
	...
	if len(meta) == 0:
		print("No metadata file was found! No metadata will be used.")
		meta = ""
	elif len(meta) > 1:
		meta_name = input("Multiple metadata files were found! Please choose the metadata file to be used. ")
		try:
			with open(meta_name) as f:
				meta = f.read()
		except FileNotFoundError:
			print("The file does not exist! No metadata will be used.")
			meta = ""
	else:
		meta = meta[0]
else:
...
def convert_level(file, file_name, meta):
...
if files_in_convert:
	...
	for i in range(len(file)):
		if os.listdir("convert")[i][-4:] == ".tmx":
			level = convert_level(file[i], os.listdir("convert")[i], meta)
			level[gmdkit.mappings.lvl_prop.NAME] = os.listdir("convert")[i].replace(".tmx", "")
			level.to_file("converted/" + os.listdir("convert")[i].replace(".tmx", "").lower() + ".gmd")
else:
	level = convert_level(file, file_name, meta)
